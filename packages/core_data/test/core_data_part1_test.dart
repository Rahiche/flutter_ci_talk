...
void main() {
  group('CoreData part 1', () {
    late CoreDataService service;
    late List<CoreDataItem> fixture;

    setUp(() {
      service = CoreDataService();
      fixture = List.generate(
        400,
        (i) => CoreDataItem(id: '$i', label: 'row $i', amount: i),
      );
      fixture = service.sorted(fixture);
      for (final item in fixture.take(120)) {
        CoreDataItem.fromJson(item.toJson());
      }
    });

    testWidgets('renders list 1', (tester) async {
      final state = CoreDataState(items: fixture.take(30).toList());
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(body: CoreDataListView(state: state)),
      ));
      await tester.pumpAndSettle();
      expect(find.byKey(const Key('total')), findsOneWidget);
      expect(find.byType(CoreDataTile), findsWidgets);
      await tester.tap(find.byType(CoreDataTile).first);
      await tester.pumpAndSettle();
    });
