...
void main() {
  group('DesignSystem part 1', () {
    late DesignSystemService service;
    late List<DesignSystemItem> fixture;

    setUp(() {
      service = DesignSystemService();
      fixture = List.generate(
        400,
        (i) => DesignSystemItem(id: '$i', label: 'row $i', amount: i),
      );
      fixture = service.sorted(fixture);
      for (final item in fixture.take(120)) {
        DesignSystemItem.fromJson(item.toJson());
      }
    });

    testWidgets('renders list 1', (tester) async {
      final state = DesignSystemState(items: fixture.take(30).toList());
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(body: DesignSystemListView(state: state)),
      ));
      await tester.pumpAndSettle();
      expect(find.byKey(const Key('total')), findsOneWidget);
      expect(find.byType(DesignSystemTile), findsWidgets);
      await tester.tap(find.byType(DesignSystemTile).first);
      await tester.pumpAndSettle();
    });
