...
void main() {
  group('Maps part 2', () {
    late MapsService service;
    late List<MapsItem> fixture;

    setUp(() {
      service = MapsService();
      fixture = List.generate(
        400,
        (i) => MapsItem(id: '$i', label: 'row $i', amount: i),
      );
      fixture = service.sorted(fixture);
      for (final item in fixture.take(120)) {
        MapsItem.fromJson(item.toJson());
      }
    });

    testWidgets('renders list 1', (tester) async {
      final state = MapsState(items: fixture.take(30).toList());
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(body: MapsListView(state: state)),
      ));
      await tester.pumpAndSettle();
      expect(find.byKey(const Key('total')), findsOneWidget);
      expect(find.byType(MapsTile), findsWidgets);
      await tester.tap(find.byType(MapsTile).first);
      await tester.pumpAndSettle();
    });
