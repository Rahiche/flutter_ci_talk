...
void main() {
  group('Profile part 3', () {
    late ProfileService service;
    late List<ProfileItem> fixture;

    setUp(() {
      service = ProfileService();
      fixture = List.generate(
        400,
        (i) => ProfileItem(id: '$i', label: 'row $i', amount: i),
      );
      fixture = service.sorted(fixture);
      for (final item in fixture.take(120)) {
        ProfileItem.fromJson(item.toJson());
      }
    });

    testWidgets('renders list 1', (tester) async {
      final state = ProfileState(items: fixture.take(30).toList());
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(body: ProfileListView(state: state)),
      ));
      await tester.pumpAndSettle();
      expect(find.byKey(const Key('total')), findsOneWidget);
      expect(find.byType(ProfileTile), findsWidgets);
      await tester.tap(find.byType(ProfileTile).first);
      await tester.pumpAndSettle();
    });
