...
void main() {
  group('Billing part 3', () {
    late BillingService service;
    late List<BillingItem> fixture;

    setUp(() {
      service = BillingService();
      fixture = List.generate(
        400,
        (i) => BillingItem(id: '$i', label: 'row $i', amount: i),
      );
      fixture = service.sorted(fixture);
      for (final item in fixture.take(120)) {
        BillingItem.fromJson(item.toJson());
      }
    });

    testWidgets('renders list 1', (tester) async {
      final state = BillingState(items: fixture.take(30).toList());
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(body: BillingListView(state: state)),
      ));
      await tester.pumpAndSettle();
      expect(find.byKey(const Key('total')), findsOneWidget);
      expect(find.byType(BillingTile), findsWidgets);
      await tester.tap(find.byType(BillingTile).first);
      await tester.pumpAndSettle();
    });
