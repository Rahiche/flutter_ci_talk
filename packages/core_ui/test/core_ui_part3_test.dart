...
void main() {
  group('CoreUi part 3', () {
    late CoreUiService service;
    late List<CoreUiItem> fixture;

    setUp(() {
      service = CoreUiService();
      fixture = List.generate(
        400,
        (i) => CoreUiItem(id: '$i', label: 'row $i', amount: i),
      );
      fixture = service.sorted(fixture);
      for (final item in fixture.take(120)) {
        CoreUiItem.fromJson(item.toJson());
      }
    });

    testWidgets('renders list 1', (tester) async {
      final state = CoreUiState(items: fixture.take(30).toList());
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(body: CoreUiListView(state: state)),
      ));
      await tester.pumpAndSettle();
      expect(find.byKey(const Key('total')), findsOneWidget);
      expect(find.byType(CoreUiTile), findsWidgets);
      await tester.tap(find.byType(CoreUiTile).first);
      await tester.pumpAndSettle();
    });
