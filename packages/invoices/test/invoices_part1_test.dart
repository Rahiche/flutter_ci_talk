...
void main() {
  group('Invoices part 1', () {
    late InvoicesService service;
    late List<InvoicesItem> fixture;

    setUp(() {
      service = InvoicesService();
      fixture = List.generate(
        400,
        (i) => InvoicesItem(id: '$i', label: 'row $i', amount: i),
      );
      fixture = service.sorted(fixture);
      for (final item in fixture.take(120)) {
        InvoicesItem.fromJson(item.toJson());
      }
    });

    testWidgets('renders list 1', (tester) async {
      final state = InvoicesState(items: fixture.take(30).toList());
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(body: InvoicesListView(state: state)),
      ));
      await tester.pumpAndSettle();
      expect(find.byKey(const Key('total')), findsOneWidget);
      expect(find.byType(InvoicesTile), findsWidgets);
      await tester.tap(find.byType(InvoicesTile).first);
      await tester.pumpAndSettle();
    });
