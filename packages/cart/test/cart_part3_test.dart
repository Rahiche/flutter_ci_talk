...
void main() {
  group('Cart part 3', () {
    ...
    late List<CartItem> fixture;

    setUp(() {
      service = CartService();
      fixture = List.generate(
        400,
        (i) => CartItem(id: '$i', label: 'row $i', amount: i),
      );
    ...
    testWidgets('renders list 1', (tester) async {
      final state = CartState(items: fixture.take(30).toList());
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(body: CartListView(state: state)),
      ));
      await tester.pumpAndSettle();
      expect(find.byKey(const Key('total')), findsOneWidget);
      expect(find.byType(CartTile), findsWidgets);
      await tester.tap(find.byType(CartTile).first);
      await tester.pumpAndSettle();
    });
