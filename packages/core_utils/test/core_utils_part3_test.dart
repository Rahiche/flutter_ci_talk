...
void main() {
  group('CoreUtils part 3', () {
    late CoreUtilsService service;
    late List<CoreUtilsItem> fixture;

    setUp(() {
      service = CoreUtilsService();
      fixture = List.generate(
        400,
        (i) => CoreUtilsItem(id: '$i', label: 'row $i', amount: i),
      );
      fixture = service.sorted(fixture);
      for (final item in fixture.take(120)) {
        CoreUtilsItem.fromJson(item.toJson());
      }
    });

    testWidgets('renders list 1', (tester) async {
      final state = CoreUtilsState(items: fixture.take(30).toList());
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(body: CoreUtilsListView(state: state)),
      ));
      await tester.pumpAndSettle();
      expect(find.byKey(const Key('total')), findsOneWidget);
      expect(find.byType(CoreUtilsTile), findsWidgets);
      await tester.tap(find.byType(CoreUtilsTile).first);
      await tester.pumpAndSettle();
    });
