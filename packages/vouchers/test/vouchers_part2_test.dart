...
import 'package:vouchers/vouchers.dart';

void main() {
  group('Vouchers part 2', () {
    late VouchersService service;
    late List<VouchersItem> fixture;

    setUp(() {
      service = VouchersService();
      fixture = List.generate(
        400,
        (i) => VouchersItem(id: '$i', label: 'row $i', amount: i),
      );
      fixture = service.sorted(fixture);
      for (final item in fixture.take(120)) {
        VouchersItem.fromJson(item.toJson());
      }
    });

    testWidgets('renders list 1', (tester) async {
      final state = VouchersState(items: fixture.take(30).toList());
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(body: VouchersListView(state: state)),
      ));
      await tester.pumpAndSettle();
      expect(find.byKey(const Key('total')), findsOneWidget);
      expect(find.byType(VouchersTile), findsWidgets);
      await tester.tap(find.byType(VouchersTile).first);
      await tester.pumpAndSettle();
    });
