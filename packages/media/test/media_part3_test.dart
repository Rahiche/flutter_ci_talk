...
void main() {
  group('Media part 3', () {
    ...
    late List<MediaItem> fixture;

    setUp(() {
      service = MediaService();
      fixture = List.generate(
        400,
        (i) => MediaItem(id: '$i', label: 'row $i', amount: i),
      );
      fixture = service.sorted(fixture);
      for (final item in fixture.take(120)) {
        MediaItem.fromJson(item.toJson());
      }
    });

    testWidgets('renders list 1', (tester) async {
      final state = MediaState(items: fixture.take(30).toList());
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(body: MediaListView(state: state)),
      ));
