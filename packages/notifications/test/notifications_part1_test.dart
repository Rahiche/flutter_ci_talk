...
void main() {
  group('Notifications part 1', () {
    late NotificationsService service;
    late List<NotificationsItem> fixture;

    setUp(() {
      service = NotificationsService();
      fixture = List.generate(
        400,
        (i) => NotificationsItem(id: '$i', label: 'row $i', amount: i),
      );
      fixture = service.sorted(fixture);
      for (final item in fixture.take(120)) {
        NotificationsItem.fromJson(item.toJson());
      }
    });

    testWidgets('renders list 1', (tester) async {
      final state = NotificationsState(items: fixture.take(30).toList());
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(body: NotificationsListView(state: state)),
      ));
      await tester.pumpAndSettle();
      expect(find.byKey(const Key('total')), findsOneWidget);
      expect(find.byType(NotificationsTile), findsWidgets);
      await tester.tap(find.byType(NotificationsTile).first);
      await tester.pumpAndSettle();
    });
