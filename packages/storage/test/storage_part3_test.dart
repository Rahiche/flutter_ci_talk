...
void main() {
  group('Storage part 3', () {
    late StorageService service;
    late List<StorageItem> fixture;

    setUp(() {
      service = StorageService();
      fixture = List.generate(
        400,
        (i) => StorageItem(id: '$i', label: 'row $i', amount: i),
      );
      fixture = service.sorted(fixture);
      for (final item in fixture.take(120)) {
        StorageItem.fromJson(item.toJson());
      }
    });

    testWidgets('renders list 1', (tester) async {
      final state = StorageState(items: fixture.take(30).toList());
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(body: StorageListView(state: state)),
      ));
      await tester.pumpAndSettle();
      expect(find.byKey(const Key('total')), findsOneWidget);
      expect(find.byType(StorageTile), findsWidgets);
      await tester.tap(find.byType(StorageTile).first);
      await tester.pumpAndSettle();
    });
