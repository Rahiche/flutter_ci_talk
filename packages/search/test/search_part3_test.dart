...
void main() {
  group('Search part 3', () {
    ...
    late List<SearchItem> fixture;

    setUp(() {
      service = SearchService();
      fixture = List.generate(
        400,
        (i) => SearchItem(id: '$i', label: 'row $i', amount: i),
      );
      fixture = service.sorted(fixture);
      for (final item in fixture.take(120)) {
        SearchItem.fromJson(item.toJson());
      }
    });

    testWidgets('renders list 1', (tester) async {
      final state = SearchState(items: fixture.take(30).toList());
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(body: SearchListView(state: state)),
      ));
      await tester.pumpAndSettle();
      expect(find.byKey(const Key('total')), findsOneWidget);
      expect(find.byType(SearchTile), findsWidgets);
      await tester.tap(find.byType(SearchTile).first);
      await tester.pumpAndSettle();
    });
