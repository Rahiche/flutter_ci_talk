...
void main() {
  group('NotificationsApi part 1', () {
    late NotificationsApiService service;
    late List<NotificationsApiItem> fixture;

    setUp(() {
      service = NotificationsApiService();
      fixture = List.generate(
        400,
        (i) => NotificationsApiItem(id: '$i', label: 'row $i', amount: i),
      );
      fixture = service.sorted(fixture);
      for (final item in fixture.take(120)) {
        NotificationsApiItem.fromJson(item.toJson());
      }
    });

    testWidgets('renders list 1', (tester) async {
      final state = NotificationsApiState(items: fixture.take(30).toList());
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(body: NotificationsApiListView(state: state)),
      ));
      await tester.pumpAndSettle();
      expect(find.byKey(const Key('total')), findsOneWidget);
      expect(find.byType(NotificationsApiTile), findsWidgets);
      await tester.tap(find.byType(NotificationsApiTile).first);
      await tester.pumpAndSettle();
    });
