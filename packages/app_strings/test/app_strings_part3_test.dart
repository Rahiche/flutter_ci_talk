...
void main() {
  group('AppStrings part 3', () {
    late AppStringsService service;
    late List<AppStringsItem> fixture;

    setUp(() {
      service = AppStringsService();
      fixture = List.generate(
        400,
        (i) => AppStringsItem(id: '$i', label: 'row $i', amount: i),
      );
      fixture = service.sorted(fixture);
      for (final item in fixture.take(120)) {
        AppStringsItem.fromJson(item.toJson());
      }
    });

    testWidgets('renders list 1', (tester) async {
      final state = AppStringsState(items: fixture.take(30).toList());
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(body: AppStringsListView(state: state)),
      ));
      await tester.pumpAndSettle();
      expect(find.byKey(const Key('total')), findsOneWidget);
      expect(find.byType(AppStringsTile), findsWidgets);
      await tester.tap(find.byType(AppStringsTile).first);
      await tester.pumpAndSettle();
    });
