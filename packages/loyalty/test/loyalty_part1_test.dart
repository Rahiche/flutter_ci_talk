...
void main() {
  group('Loyalty part 1', () {
    late LoyaltyService service;
    late List<LoyaltyItem> fixture;

    setUp(() {
      service = LoyaltyService();
      fixture = List.generate(
        400,
        (i) => LoyaltyItem(id: '$i', label: 'row $i', amount: i),
      );
      fixture = service.sorted(fixture);
      for (final item in fixture.take(120)) {
        LoyaltyItem.fromJson(item.toJson());
      }
    });

    testWidgets('renders list 1', (tester) async {
      final state = LoyaltyState(items: fixture.take(30).toList());
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(body: LoyaltyListView(state: state)),
      ));
      await tester.pumpAndSettle();
      expect(find.byKey(const Key('total')), findsOneWidget);
      expect(find.byType(LoyaltyTile), findsWidgets);
      await tester.tap(find.byType(LoyaltyTile).first);
      await tester.pumpAndSettle();
    });
