...
void main() {
  group('Wishlist part 1', () {
    late WishlistService service;
    late List<WishlistItem> fixture;

    setUp(() {
      service = WishlistService();
      fixture = List.generate(
        400,
        (i) => WishlistItem(id: '$i', label: 'row $i', amount: i),
      );
      fixture = service.sorted(fixture);
      for (final item in fixture.take(120)) {
        WishlistItem.fromJson(item.toJson());
      }
    });

    testWidgets('renders list 1', (tester) async {
      final state = WishlistState(items: fixture.take(30).toList());
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(body: WishlistListView(state: state)),
      ));
      await tester.pumpAndSettle();
      expect(find.byKey(const Key('total')), findsOneWidget);
      expect(find.byType(WishlistTile), findsWidgets);
      await tester.tap(find.byType(WishlistTile).first);
      await tester.pumpAndSettle();
    });
