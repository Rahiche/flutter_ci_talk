...
void main() {
  group('Gallery part 1', () {
    late GalleryService service;
    late List<GalleryItem> fixture;

    setUp(() {
      service = GalleryService();
      fixture = List.generate(
        400,
        (i) => GalleryItem(id: '$i', label: 'row $i', amount: i),
      );
      fixture = service.sorted(fixture);
      for (final item in fixture.take(120)) {
        GalleryItem.fromJson(item.toJson());
      }
    });

    testWidgets('renders list 1', (tester) async {
      final state = GalleryState(items: fixture.take(30).toList());
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(body: GalleryListView(state: state)),
      ));
