...
void main() {
  group('Stories part 2', () {
    late StoriesService service;
    late List<StoriesItem> fixture;

    setUp(() {
      service = StoriesService();
      fixture = List.generate(
        400,
        (i) => StoriesItem(id: '$i', label: 'row $i', amount: i),
      );
      fixture = service.sorted(fixture);
      for (final item in fixture.take(120)) {
        StoriesItem.fromJson(item.toJson());
      }
    });

    testWidgets('renders list 1', (tester) async {
      final state = StoriesState(items: fixture.take(30).toList());
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(body: StoriesListView(state: state)),
      ));
      await tester.pumpAndSettle();
      expect(find.byKey(const Key('total')), findsOneWidget);
      expect(find.byType(StoriesTile), findsWidgets);
      await tester.tap(find.byType(StoriesTile).first);
      await tester.pumpAndSettle();
    });
