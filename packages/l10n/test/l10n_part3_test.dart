...
void main() {
  group('L10n part 3', () {
    late L10nService service;
    late List<L10nItem> fixture;

    setUp(() {
      service = L10nService();
      fixture = List.generate(
        400,
        (i) => L10nItem(id: '$i', label: 'row $i', amount: i),
      );
      fixture = service.sorted(fixture);
      for (final item in fixture.take(120)) {
        L10nItem.fromJson(item.toJson());
      }
    });

    testWidgets('renders list 1', (tester) async {
      final state = L10nState(items: fixture.take(30).toList());
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(body: L10nListView(state: state)),
      ));
      await tester.pumpAndSettle();
      expect(find.byKey(const Key('total')), findsOneWidget);
      expect(find.byType(L10nTile), findsWidgets);
      await tester.tap(find.byType(L10nTile).first);
      await tester.pumpAndSettle();
    });
