...
void main() {
  group('Settings part 2', () {
    ...
    late List<SettingsItem> fixture;

    setUp(() {
      service = SettingsService();
      fixture = List.generate(
        400,
        (i) => SettingsItem(id: '$i', label: 'row $i', amount: i),
      );
      fixture = service.sorted(fixture);
      for (final item in fixture.take(120)) {
        SettingsItem.fromJson(item.toJson());
      }
    });

    testWidgets('renders list 1', (tester) async {
      final state = SettingsState(items: fixture.take(30).toList());
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(body: SettingsListView(state: state)),
      ));
