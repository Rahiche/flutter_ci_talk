...
void main() {
  group('Feed part 3', () {
    ...
    late List<FeedItem> fixture;

    setUp(() {
      service = FeedService();
      fixture = List.generate(
        400,
        (i) => FeedItem(id: '$i', label: 'row $i', amount: i),
      );
      fixture = service.sorted(fixture);
      for (final item in fixture.take(120)) {
        FeedItem.fromJson(item.toJson());
      }
    });

    testWidgets('renders list 1', (tester) async {
      final state = FeedState(items: fixture.take(30).toList());
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(body: FeedListView(state: state)),
      ));
      await tester.pumpAndSettle();
      expect(find.byKey(const Key('total')), findsOneWidget);
      expect(find.byType(FeedTile), findsWidgets);
      await tester.tap(find.byType(FeedTile).first);
      await tester.pumpAndSettle();
    });
