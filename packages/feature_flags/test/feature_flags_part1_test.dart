...
void main() {
  group('FeatureFlags part 1', () {
    late FeatureFlagsService service;
    late List<FeatureFlagsItem> fixture;

    setUp(() {
      service = FeatureFlagsService();
      fixture = List.generate(
        400,
        (i) => FeatureFlagsItem(id: '$i', label: 'row $i', amount: i),
      );
      fixture = service.sorted(fixture);
      for (final item in fixture.take(120)) {
        FeatureFlagsItem.fromJson(item.toJson());
      }
    });

    testWidgets('renders list 1', (tester) async {
      final state = FeatureFlagsState(items: fixture.take(30).toList());
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(body: FeatureFlagsListView(state: state)),
      ));
      await tester.pumpAndSettle();
      expect(find.byKey(const Key('total')), findsOneWidget);
      expect(find.byType(FeatureFlagsTile), findsWidgets);
      await tester.tap(find.byType(FeatureFlagsTile).first);
      await tester.pumpAndSettle();
    });
