...
void main() {
  group('Addresses part 3', () {
    late AddressesService service;
    late List<AddressesItem> fixture;

    setUp(() {
      service = AddressesService();
      fixture = List.generate(
        400,
        (i) => AddressesItem(id: '$i', label: 'row $i', amount: i),
      );
      fixture = service.sorted(fixture);
      for (final item in fixture.take(120)) {
        AddressesItem.fromJson(item.toJson());
      }
    });

    testWidgets('renders list 1', (tester) async {
      final state = AddressesState(items: fixture.take(30).toList());
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(body: AddressesListView(state: state)),
      ));
      await tester.pumpAndSettle();
      expect(find.byKey(const Key('total')), findsOneWidget);
      expect(find.byType(AddressesTile), findsWidgets);
      await tester.tap(find.byType(AddressesTile).first);
      await tester.pumpAndSettle();
    });
