...
import 'package:analytics_events/analytics_events.dart';

void main() {
  group('AnalyticsEvents part 3', () {
    late AnalyticsEventsService service;
    late List<AnalyticsEventsItem> fixture;

    setUp(() {
      service = AnalyticsEventsService();
      fixture = List.generate(
        400,
        (i) => AnalyticsEventsItem(id: '$i', label: 'row $i', amount: i),
      );
      fixture = service.sorted(fixture);
      for (final item in fixture.take(120)) {
        AnalyticsEventsItem.fromJson(item.toJson());
      }
    });

    testWidgets('renders list 1', (tester) async {
      final state = AnalyticsEventsState(items: fixture.take(30).toList());
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(body: AnalyticsEventsListView(state: state)),
      ));
      await tester.pumpAndSettle();
      expect(find.byKey(const Key('total')), findsOneWidget);
      expect(find.byType(AnalyticsEventsTile), findsWidgets);
      await tester.tap(find.byType(AnalyticsEventsTile).first);
      await tester.pumpAndSettle();
    });
