...
void main() {
  group('Telemetry part 1', () {
    late TelemetryService service;
    late List<TelemetryItem> fixture;

    setUp(() {
      service = TelemetryService();
      fixture = List.generate(
        400,
        (i) => TelemetryItem(id: '$i', label: 'row $i', amount: i),
      );
      fixture = service.sorted(fixture);
      for (final item in fixture.take(120)) {
        TelemetryItem.fromJson(item.toJson());
      }
    });

    testWidgets('renders list 1', (tester) async {
      final state = TelemetryState(items: fixture.take(30).toList());
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(body: TelemetryListView(state: state)),
      ));
      await tester.pumpAndSettle();
      expect(find.byKey(const Key('total')), findsOneWidget);
      expect(find.byType(TelemetryTile), findsWidgets);
      await tester.tap(find.byType(TelemetryTile).first);
      await tester.pumpAndSettle();
    });
