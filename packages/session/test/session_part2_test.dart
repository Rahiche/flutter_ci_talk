...
void main() {
  group('Session part 2', () {
    late SessionService service;
    late List<SessionItem> fixture;

    setUp(() {
      service = SessionService();
      fixture = List.generate(
        400,
        (i) => SessionItem(id: '$i', label: 'row $i', amount: i),
      );
      fixture = service.sorted(fixture);
      for (final item in fixture.take(120)) {
        SessionItem.fromJson(item.toJson());
      }
    });

    testWidgets('renders list 1', (tester) async {
      final state = SessionState(items: fixture.take(30).toList());
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(body: SessionListView(state: state)),
      ));
      await tester.pumpAndSettle();
      expect(find.byKey(const Key('total')), findsOneWidget);
      expect(find.byType(SessionTile), findsWidgets);
      await tester.tap(find.byType(SessionTile).first);
      await tester.pumpAndSettle();
    });
