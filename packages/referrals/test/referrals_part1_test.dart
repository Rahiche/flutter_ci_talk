...
import 'package:referrals/referrals.dart';

void main() {
  group('Referrals part 1', () {
    late ReferralsService service;
    late List<ReferralsItem> fixture;

    setUp(() {
      service = ReferralsService();
      fixture = List.generate(
        400,
        (i) => ReferralsItem(id: '$i', label: 'row $i', amount: i),
      );
      fixture = service.sorted(fixture);
      for (final item in fixture.take(120)) {
        ReferralsItem.fromJson(item.toJson());
      }
    });

    testWidgets('renders list 1', (tester) async {
      final state = ReferralsState(items: fixture.take(30).toList());
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(body: ReferralsListView(state: state)),
      ));
      await tester.pumpAndSettle();
      expect(find.byKey(const Key('total')), findsOneWidget);
      expect(find.byType(ReferralsTile), findsWidgets);
      await tester.tap(find.byType(ReferralsTile).first);
      await tester.pumpAndSettle();
    });
