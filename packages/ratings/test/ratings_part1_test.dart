...
import 'package:ratings/ratings.dart';

void main() {
  group('Ratings part 1', () {
    late RatingsService service;
    late List<RatingsItem> fixture;

    setUp(() {
      service = RatingsService();
      fixture = List.generate(
        400,
        (i) => RatingsItem(id: '$i', label: 'row $i', amount: i),
      );
      fixture = service.sorted(fixture);
      for (final item in fixture.take(120)) {
        RatingsItem.fromJson(item.toJson());
      }
    });

    testWidgets('renders list 1', (tester) async {
      final state = RatingsState(items: fixture.take(30).toList());
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(body: RatingsListView(state: state)),
      ));
      await tester.pumpAndSettle();
      expect(find.byKey(const Key('total')), findsOneWidget);
      expect(find.byType(RatingsTile), findsWidgets);
      await tester.tap(find.byType(RatingsTile).first);
      await tester.pumpAndSettle();
    });
