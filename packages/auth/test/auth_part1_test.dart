...
void main() {
  group('Auth part 1', () {
    ...
    late List<AuthItem> fixture;

    setUp(() {
      service = AuthService();
      fixture = List.generate(
        400,
        (i) => AuthItem(id: '$i', label: 'row $i', amount: i),
      );
      fixture = service.sorted(fixture);
      for (final item in fixture.take(120)) {
        AuthItem.fromJson(item.toJson());
      }
    });

    testWidgets('renders list 1', (tester) async {
      final state = AuthState(items: fixture.take(30).toList());
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(body: AuthListView(state: state)),
      ));
      await tester.pumpAndSettle();
      expect(find.byKey(const Key('total')), findsOneWidget);
      expect(find.byType(AuthTile), findsWidgets);
      await tester.tap(find.byType(AuthTile).first);
      await tester.pumpAndSettle();
    });
