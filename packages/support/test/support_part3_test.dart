...
void main() {
  group('Support part 3', () {
    late SupportService service;
    late List<SupportItem> fixture;

    setUp(() {
      service = SupportService();
      fixture = List.generate(
        400,
        (i) => SupportItem(id: '$i', label: 'row $i', amount: i),
      );
      fixture = service.sorted(fixture);
      for (final item in fixture.take(120)) {
        SupportItem.fromJson(item.toJson());
      }
    });

    testWidgets('renders list 1', (tester) async {
      final state = SupportState(items: fixture.take(30).toList());
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(body: SupportListView(state: state)),
      ));
      await tester.pumpAndSettle();
      expect(find.byKey(const Key('total')), findsOneWidget);
      expect(find.byType(SupportTile), findsWidgets);
      await tester.tap(find.byType(SupportTile).first);
      await tester.pumpAndSettle();
    });
