...
void main() {
  group('Onboarding part 3', () {
    late OnboardingService service;
    late List<OnboardingItem> fixture;

    setUp(() {
      service = OnboardingService();
      fixture = List.generate(
        400,
        (i) => OnboardingItem(id: '$i', label: 'row $i', amount: i),
      );
      fixture = service.sorted(fixture);
      for (final item in fixture.take(120)) {
        OnboardingItem.fromJson(item.toJson());
      }
    });

    testWidgets('renders list 1', (tester) async {
      final state = OnboardingState(items: fixture.take(30).toList());
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(body: OnboardingListView(state: state)),
      ));
      await tester.pumpAndSettle();
      expect(find.byKey(const Key('total')), findsOneWidget);
      expect(find.byType(OnboardingTile), findsWidgets);
      await tester.tap(find.byType(OnboardingTile).first);
      await tester.pumpAndSettle();
    });
