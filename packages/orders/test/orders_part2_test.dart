...
void main() {
  group('Orders part 2', () {
    late OrdersService service;
    late List<OrdersItem> fixture;

    setUp(() {
      service = OrdersService();
      fixture = List.generate(
        400,
        (i) => OrdersItem(id: '$i', label: 'row $i', amount: i),
      );
      fixture = service.sorted(fixture);
      for (final item in fixture.take(120)) {
        OrdersItem.fromJson(item.toJson());
      }
    });

    testWidgets('renders list 1', (tester) async {
      final state = OrdersState(items: fixture.take(30).toList());
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(body: OrdersListView(state: state)),
      ));
      await tester.pumpAndSettle();
      expect(find.byKey(const Key('total')), findsOneWidget);
      expect(find.byType(OrdersTile), findsWidgets);
      await tester.tap(find.byType(OrdersTile).first);
      await tester.pumpAndSettle();
    });
