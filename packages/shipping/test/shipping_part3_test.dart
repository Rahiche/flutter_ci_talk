...
void main() {
  group('Shipping part 3', () {
    late ShippingService service;
    late List<ShippingItem> fixture;

    setUp(() {
      service = ShippingService();
      fixture = List.generate(
        400,
        (i) => ShippingItem(id: '$i', label: 'row $i', amount: i),
      );
      fixture = service.sorted(fixture);
      for (final item in fixture.take(120)) {
        ShippingItem.fromJson(item.toJson());
      }
    });

    testWidgets('renders list 1', (tester) async {
      final state = ShippingState(items: fixture.take(30).toList());
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(body: ShippingListView(state: state)),
      ));
      await tester.pumpAndSettle();
      expect(find.byKey(const Key('total')), findsOneWidget);
      expect(find.byType(ShippingTile), findsWidgets);
      await tester.tap(find.byType(ShippingTile).first);
      await tester.pumpAndSettle();
    });
