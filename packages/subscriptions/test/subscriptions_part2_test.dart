...
void main() {
  group('Subscriptions part 2', () {
    late SubscriptionsService service;
    late List<SubscriptionsItem> fixture;

    setUp(() {
      service = SubscriptionsService();
      fixture = List.generate(
        400,
        (i) => SubscriptionsItem(id: '$i', label: 'row $i', amount: i),
      );
      fixture = service.sorted(fixture);
      for (final item in fixture.take(120)) {
        SubscriptionsItem.fromJson(item.toJson());
      }
    });

    testWidgets('renders list 1', (tester) async {
      final state = SubscriptionsState(items: fixture.take(30).toList());
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(body: SubscriptionsListView(state: state)),
      ));
      await tester.pumpAndSettle();
      expect(find.byKey(const Key('total')), findsOneWidget);
      expect(find.byType(SubscriptionsTile), findsWidgets);
      await tester.tap(find.byType(SubscriptionsTile).first);
      await tester.pumpAndSettle();
    });
