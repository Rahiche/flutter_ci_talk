...
void main() {
  group('Calendar part 1', () {
    late CalendarService service;
    late List<CalendarItem> fixture;

    setUp(() {
      service = CalendarService();
      fixture = List.generate(
        400,
        (i) => CalendarItem(id: '$i', label: 'row $i', amount: i),
      );
      fixture = service.sorted(fixture);
      for (final item in fixture.take(120)) {
        CalendarItem.fromJson(item.toJson());
      }
    });

    testWidgets('renders list 1', (tester) async {
      final state = CalendarState(items: fixture.take(30).toList());
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(body: CalendarListView(state: state)),
      ));
      await tester.pumpAndSettle();
      expect(find.byKey(const Key('total')), findsOneWidget);
      expect(find.byType(CalendarTile), findsWidgets);
      await tester.tap(find.byType(CalendarTile).first);
      await tester.pumpAndSettle();
    });
