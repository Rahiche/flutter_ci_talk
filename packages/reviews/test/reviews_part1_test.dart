...
void main() {
  group('Reviews part 1', () {
    late ReviewsService service;
    late List<ReviewsItem> fixture;

    setUp(() {
      service = ReviewsService();
      fixture = List.generate(
        400,
        (i) => ReviewsItem(id: '$i', label: 'row $i', amount: i),
      );
      fixture = service.sorted(fixture);
      for (final item in fixture.take(120)) {
        ReviewsItem.fromJson(item.toJson());
      }
    });

    testWidgets('renders list 1', (tester) async {
      final state = ReviewsState(items: fixture.take(30).toList());
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(body: ReviewsListView(state: state)),
      ));
      await tester.pumpAndSettle();
      expect(find.byKey(const Key('total')), findsOneWidget);
      expect(find.byType(ReviewsTile), findsWidgets);
      await tester.tap(find.byType(ReviewsTile).first);
      await tester.pumpAndSettle();
    });
