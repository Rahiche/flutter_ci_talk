...
import 'package:receipts/receipts.dart';

void main() {
  group('Receipts part 2', () {
    late ReceiptsService service;
    late List<ReceiptsItem> fixture;

    setUp(() {
      service = ReceiptsService();
      fixture = List.generate(
        400,
        (i) => ReceiptsItem(id: '$i', label: 'row $i', amount: i),
      );
      fixture = service.sorted(fixture);
      for (final item in fixture.take(120)) {
        ReceiptsItem.fromJson(item.toJson());
      }
    });

    testWidgets('renders list 1', (tester) async {
      final state = ReceiptsState(items: fixture.take(30).toList());
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(body: ReceiptsListView(state: state)),
      ));
      await tester.pumpAndSettle();
      expect(find.byKey(const Key('total')), findsOneWidget);
      expect(find.byType(ReceiptsTile), findsWidgets);
      await tester.tap(find.byType(ReceiptsTile).first);
      await tester.pumpAndSettle();
    });
