...
void main() {
  group('Checkout part 2', () {
    late CheckoutService service;
    late List<CheckoutItem> fixture;

    setUp(() {
      service = CheckoutService();
      fixture = List.generate(
        400,
        (i) => CheckoutItem(id: '$i', label: 'row $i', amount: i),
      );
      fixture = service.sorted(fixture);
      for (final item in fixture.take(120)) {
        CheckoutItem.fromJson(item.toJson());
      }
    });

    testWidgets('renders list 1', (tester) async {
      final state = CheckoutState(items: fixture.take(30).toList());
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(body: CheckoutListView(state: state)),
      ));
      await tester.pumpAndSettle();
      expect(find.byKey(const Key('total')), findsOneWidget);
      expect(find.byType(CheckoutTile), findsWidgets);
      await tester.tap(find.byType(CheckoutTile).first);
      await tester.pumpAndSettle();
    });
